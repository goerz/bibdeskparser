...
[journal_macros]             # journal-name-to-macro mappings for import
...
The `[journal_macros]` table (here: a curated set of physics and
mathematics journals with their ISO 4 abbreviations) makes `import`
and `add` resolve any of the listed journal names to a stable macro,
defining the `@string` in the library on first use; the two list
values show how publisher spelling variants canonicalize to one
macro. The `[initials.journal]` entries pin the citation-key initials
