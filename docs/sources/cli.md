...
- `--update FILE` -- rewrite the exported FILE (which must exist and be plain BibTeX, not a BibDesk database), refreshing it from the library: the given KEYs (without KEYs: every key in FILE that the library knows) take the library's current values, KEYs not yet in FILE are appended, and everything else -- unknown entries, comments, `@string` definitions -- is kept; nothing is ever removed. That holds field by field: a refreshed entry keeps the fields it has in FILE, in FILE's own field order, and the field selection only adds to them. The `--unicode`/`--expand-strings`/`--preprint` options default to the FILE's own recorded or detected options.
