...
A source names a place to take text from, and is evaluated separately for every entry. It is either an entry field, such as `title`, `abstract`, or `note`, whose value is then used directly, or an [entry asset class](external-assets), in which case the content of that entry's companion file is read from disk. Asset classes are declared in the `[assets]` table of the same configuration file; should a name be declared as both, the asset takes precedence. Three kinds of name are a configuration error, reported when the index is built: one that is neither a field nor an asset class, a *library* asset class, which is one text for the whole library and would contribute the same words to every row, and a *directory*-valued asset class, which resolves to a directory and so holds no text at all. A file that is not UTF-8 text is skipped with a warning naming it and its entry, since one such file should not abort a whole build.

An index name becomes a filename, so it is restricted to letters, digits, hyphens, and underscores, and two names differing only in case are rejected as one, because the filesystem BibDesk runs on does not tell them apart.
...
The criterion needs a bulk to locate, so it needs a library. On a handful of entries the median and the MAD are estimated from a handful of numbers, and an ordinary above-median cosine clears any threshold; a library of that size is better served by reading all of it. A bulk with no spread at all is the other degenerate case, and it arises whenever a source repeats boilerplate over half the library or more: the MAD is then exactly 0 and a cut at the bare median would admit everything above it. The standard deviation still sees the rest of the distribution and stands in for the MAD there, erring toward admitting nothing because the very values in question inflate it. Where every row is identical, nothing can stand out from anything and no entry matches.
...
The `limit` applies to what comes out, not to either ranking on its way in: an entry just past it semantically that the lexical leg also ranks highly is precisely the consensus the method is built to promote. The remaining behavior follows from the formula. An entry found by only the lexical ranking still appears, whether the index omits it entirely or holds a row whose cosine the match cut rejected. Its reported cosine is `null` in both cases: a below-cut cosine is a number the procedure has just declined to treat as a match, and printing it would present a non-match as one. A query with no semantic matches reduces to the plain lexical order. And the fused order need not decrease monotonically in the reported cosine, since the cosine is not what determined the sorting.
...
- `default.npz`, the matrix of unit vectors, one row per citation key, together with the citation keys themselves in row order. For a thousand entries it occupies about 1.5 MB.
- `default.json`, the manifest, recording for each key the sources that actually contributed and a hash of the embedded text, together with the index *fingerprint*: the source list, the model, the model file, the vector dimension, the batch size, the `fastembed` version, the token budget, and a version number for the chunking and pooling arithmetic that the model's own identity cannot see.

The two files are one artifact written in two steps, so the loader checks that the keys stored beside the matrix are the manifest's keys. An interruption between the two writes would otherwise be able to leave a new matrix beside an old manifest with the same number of rows, and every vector would belong to the wrong entry.
...
Each file is written beside its destination and renamed onto it, so an interrupted build leaves the index that was already there rather than a half-written one. An index that cannot be read at all, however it came to be that way, counts as one that has not been built, which is what allows the rebuild that every other error message recommends to be the command that repairs it.

Refreshing is precise and proceeds per entry. A key whose text hashes to the stored value retains its vector, a changed or new key is re-embedded, and a key the library no longer holds is dropped, which is also what prevents a [`rekey`](cli-rekey) from leaving a stray row behind. A change to the fingerprint, whether a different source list, a different model, a different `fastembed` release, or a change to the chunking, rebuilds that index as a whole, so that vectors produced by different pipelines are never mixed within one matrix. Querying such an index is refused outright rather than warned about, since a search embeds its query with the model of the moment and multiplying that vector against a matrix something else wrote is silently wrong wherever the two agree on the number of dimensions. Including the source list in the fingerprint is what allows an edited index definition to register as a single rebuild rather than as a thousand individual hash mismatches.

Searching or scoring with an index whose *text* has fallen behind the library still works, using the stored vectors, and warns while naming the keys concerned. A row belonging to an entry that has since been deleted is the one case that cannot be honored that way. There is no entry to return for it and no other command would accept its citation key, so search passes over it, and scoring leaves it out of the neighborhood it averages, of the neighbors it reports, and of the probes it calibrates against, since a paper the library no longer holds is not one of the library's own papers. Reproducibility rests on the computation graph being a frozen artifact: a pinned `fastembed` release produces identical vectors from run to run, and differences between machines amount to floating-point noise in the last place, far too small to reorder a ranking.
