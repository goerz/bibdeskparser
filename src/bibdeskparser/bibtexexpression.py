"""
The ``pyparsing`` core for :mod:`.bparser`.
"""
...
def field_to_pair(string_, location, token):
    ...
    if isinstance(value, pp.ParseResults):
        ...
        # Changelog).
        value = value[0]
    return (field.get('FieldName'), strip_after_new_lines(value))
...
class BibtexExpression:
    ...
    * implicit_comment: an implicit comment
    ...
    def __init__(self):
        ...
        def citekeyParseAction(string_, location, token):
            ...
            if len(key) < 1:
                raise self.ParseException(
                    string_,
                    loc=location,
                    msg="Empty citekeys are not allowed.",
                )
        ...
        self.entry = (
            entry_type + in_braces_or_pars(key + pp.Suppress(',') + field_list)
        )('Entry')
        ...
        not_an_implicit_comment = (
            pp.LineEnd() + pp.Literal('@')
        ) | pp.StringEnd()
        self.explicit_comment = (
            pp.Suppress(comment_line_start)
            + pp.originalTextFor(
                pp.SkipTo(not_an_implicit_comment), asString=True
            )
        )('ExplicitComment')
        ...
        self.preamble_decl = (
            pp.Suppress(preamble_start) + in_braces_or_pars(value)
        )('PreambleDeclaration')
