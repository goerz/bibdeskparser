...
def edit_entries(entries, library=None, editor=None):
    ...
    * With a callable editor, there is no prompt: a {exc}`ValueError`
    ...
    # field missing from the buffer is *deleted* on merge, so neither
    # the minimal field selection nor the preprint-only export
    # transformation may apply here.
    text = export_entries(
        entries, strings=strings, fields="full", preprint="stored"
    )
