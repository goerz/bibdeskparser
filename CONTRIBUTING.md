...
Markdown prose is not hard-wrapped: a paragraph, a list item, or a table row is one long line, however wide, so that editing a sentence never reflows the lines around it. Only fenced code blocks keep their own line breaks (and stay within a width that renders without horizontal scrolling). This applies to every `.md` file in the repository, including the ones under `docs/sources/`.
