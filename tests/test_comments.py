...
from bibdeskparser.bwriter import to_bibtex
...
class TestParseComment(unittest.TestCase):
    ...
    def test_multiline_comments(self):
        with open(
            'tests/data/multiline_comments.bib'
        ) as bibfile:
            bib = BibTexParser(bibfile.read())
        expected = [
...
""",
    ...
    def test_comments_percentage(self):
        with open(
            'tests/data/comments_percentage.bib', 'r'
        ) as bibfile:
            bib = BibTexParser(bibfile.read())
    ...
    def test_comments_percentage_nocoma(self):
        with open(
            'tests/data/comments_percentage_nolastcoma.bib', 'r'
        ) as bibfile:
            bib = BibTexParser(bibfile.read())
            res = bib.get_entry_list()
        expected = [
...
class TestWriteComment(unittest.TestCase):
    def test_comment_write(self):
        ...
        with open(
            'tests/data/comments_only_output.bib'
        ) as bibfile:
    ...
    def test_multiple_entries(self):
        with open(
            ...
            bib = BibTexParser(bibfile.read())
        with open(
            'tests/data/multiple_entries_and_comments_output.bib'
        ) as bibfile:
            expected = bibfile.read()
        result = to_bibtex(bib)
        self.assertEqual(result, expected)
